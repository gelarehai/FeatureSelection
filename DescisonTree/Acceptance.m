function [ Best_Solution,Xt,Index,sortNeighbours,IX,accuracyXb,AccuracyXt,Best_Accuracy,Xb] = Acceptance(Neighbors,label_vector, instance_matrix,nDataset,mDataset,Xb,Tc,label_idx);
[b,z]=size(Neighbors);
[mm,n] = size(Neighbors);
%% Evaluate Xb
...
for K = 1:k
 ...
 %% Tarin the model
modelTrain  = fitctree (instance_matrix_train_Xt,label_vector_train_Xt);
%% Testing Set
...
for K = 1:k
 ...
 %% Tarin the model 
modelTrain = fitctree (instance_matrix_train_Xb,label_vector_train_Xb);
%% Testing Set
selected_test_set = test(cp,K);
y = 1;
for j = 1:mDataset
...
%% Checking th Acceptance Criteria
[s,z]=size(Neighbors);
for i=1:s
     p=rand
