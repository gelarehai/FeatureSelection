...
%% Filter method to score (rank)features based on most frequent class label (pivot_class4filter) 
...
[Best_Solution,Xt,Index,sortNeighbours,IX,accuracyXb,AccuracyXt,Best_Accuracy,Xb] = Acceptance(Neighbors,label_vector, instance_matrix,nDataset,mDataset,Xb,Tc,label_idx)
answer (iter,:)= Best_Accuracy(1,1);
Best_Solutions (iter,:) = Best_Solution(1,:);
SelectedFeatures(iter,1) = sum (Best_Solution(1,:));
iter
end
 FinalSubset = Best_Solutions(iter,:); 
 %% Final Accuracy
